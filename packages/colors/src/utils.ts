...
export function convertToPercentage(value: string) {
  if (+value <= 1) {
    value = +value * 100 + '%';
  }
...
export function setRangeForRgb(value: number, max: number) {
  if (isOnePointZero(value)) {
    value = '100%';
  }

  const percent = isPercentage(value);
  value = mathMin(max as number, mathMax(0, parseFloat(value)));
